...
admin15() {
  ...
  async gambar() {
         let title = this.payload.join('  ');
                this._sendFileByUrl(this.messages,`https://www.google.co.id/search?q=${title}`); {
    let media = 1;
    ...
    let head = await unirest.head(uri,async (res) => {
      ...
      return this._sendFile(this.messages,locationFile,media);
    });
  }
}
